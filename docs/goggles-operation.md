...
temperature sensor on top reports hot: top fan goes to max speed;

temperature sensor on left/right reports hot: left/right fan goes to max speed;
...
Note: It is convenient to start/stop DVR with ELRS backpack function. See .
...
The player will ignore files that are less than 5MB.
